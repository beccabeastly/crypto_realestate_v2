...
This crypto realestate tool uses an `.env` file to store FTX API key and secret keys. To use this tool, create and login to [FTX US exchange](https://ftx.us/profile) to access your trading API keys. Then create an `.env` file to your remote repository on your local environment with the following syntex below:
```
...
Using hvplot to visualize real estate property listing across the United States. User can use interactive widgets to pan, zoom, and hover over marked points for more property information:
